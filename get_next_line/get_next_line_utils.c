...
/*   Updated: 2022/02/01 17:22:49 by amoriah          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_substr_(char const *s, unsigned int start, size_t len)
{
	...
	i = 0;
	if (!s || len <= 0 || start >= ft_strlen_(s))
		return (NULL);
	p = (char *)s;
	len_sub = ft_strlen_(p) - start;
	if (len_sub > len)
		len_sub = len;
	subs = ft_calloc_((len_sub + 1), sizeof(char));
	if (!subs)
		return (NULL);
	while (len_sub--)
		subs[i++] = p[start++];
...
char	*ft_strjoin_(char const *s1, char const *s2)
{
	...
	if (!s1 || !s2)
		return (NULL);
	len_s1 = ft_strlen_((char *)s1);
	len_s2 = ft_strlen_((char *)s2);
